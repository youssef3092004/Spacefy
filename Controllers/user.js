...
export const getAllUserWithSpecificRoleForBusiness = async (req, res, next) => {
  try {
    const allowedRoles = ["STAFF", "ADMIN"];
    const { page, limit, skip, sort, order } = pagination(req);
    const { businessId } = req.params;
    const roleParam = req.params.roleName ?? req.params.role;
    if (!businessId) {
      return next(new AppError("Business ID is required", 400));
    }
    let roleFilter = null;
    if (roleParam) {
      const roleName = String(roleParam).toUpperCase();
      if (!allowedRoles.includes(roleName)) {
        return next(
          new AppError(
            "Invalid role specified. Allowed roles are STAFF and ADMIN.",
            400,
          ),
        );
      }
      ...
      roleFilter = { in: roleRecords.map((item) => item.id) };
    }
    ...
    if (!staff.length) {
      return next(new AppError("No staff found for this business", 404));
    }
    const totalPages = Math.ceil(total / limit);
    res.status(200).json({
      success: true,
